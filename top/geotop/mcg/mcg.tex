...
\end{remark}
   ...
   we define $\left( S_1 \# D^2, I_1 \# I \right) 
   := \left( S_1, I_1 \right) $ and
   $\left( D^2 \# S_2, I \# I_2 \right) :=
   \left( S_2, I_2 \right) $.\\
...
The monoidal structure $\# $ on $\mathcal{M}_2$ is defined
...
$D^{\# m} = D_1 \# \ldots \# \left( D_i \# D_{i+1} \right) 
\# \ldots \# D_m$, where subscripts are used to enumerate
the disks. The underlying surface, by construction, will
be $X_m$. Let
$a_i$ defnote the isotopy class of a curve in the interior
$D_i \# D_{i+1} \approx
...
\begin{figure}[H]
    ...
    \caption{The curve $a_i$ in $D_i \# D_{i+1}$}
\end{figure}
...
$T_i \in \Aut_{\mathcal{M}_2}\left( D^{\# m} \right) $ 
...
inverses $T_{i}^{-1}$.\\
\linebreak
If we add a disk to either side of $D^{\# m}$, we get
\[
T_i \# \id_{D} = T_i \quad \text{and} \quad
\id_{D}\# T_i = T_{i+1}
\] 
in $\Aut_{\mathcal{M}_2}\left( D^{\# m+1} \right) $.
Hence this gives the relation
\[
    \left( T_1^{-1} \# \id_D \right) \left( \id_D \#
    T_1^{-1} \right) \left( T_1^{-1} \# id_D \right) 
    = 
    \left( \id_D \# T_1^{-1} \right) 
    \left( T_1^{-1} \# \id_D \right) 
    \left( \id_D \# T_1^{-1} \right) 
\] 
in $\Aut_{\mathcal{M}_2} \left( D^{\# 3} \right) $,
meaning that $T_1^{-1}$ is a Yang-Baxter element. This
yields a monoidal functor
\[
\Phi = \Phi_{D, T_1^{-1}} \colon
\left( B, \otimes \right) 
\to \left( \mathcal{M}_2, \# \right) 
\] 
...
D^{\# i-1} \#  T_1^{-1} \# D^{\# n - i -1} =
T_i^{-1} \in 
\Aut_{\mathcal{M}_2}\left( D^{\# m} \right) 
