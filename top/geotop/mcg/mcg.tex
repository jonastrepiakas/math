...
of surfaces and its connection to braid groups via the Birman-Hilden
theorem. This is done following the book 'A Primer on Mapping Class Groups' by
Farb and Margalit \cite{Farb-Margalit}. In particular, we start by
...
We will prove the Birman-Hilden theorem which precisely says
...
numbers of curves which is a \todo{} 
...
\begin{proof}
    ...
    covering maps are local homeomorphisms. \todo{Ask
    Erik}. If a point of
    ...
    Now we claim that
    $p(D)$ is an embedded disk in $S$.
    Suppose $x,y \in D$ with
    $p(x)=p(y)$. Then
    $x = \varphi (y)$ for some deck transformation
    $\varphi $. Since $p$ is injective on
    $\partial D$, either
    $\varphi \left( \partial D \right) \cap
    \partial D = \varnothing$ or
    $\varphi = \id$ since
    otherwise, if $\varphi \neq \id$ and
    $\varphi \left( \partial D \right) \cap
    \partial D \neq \varnothing$, then
    choosing a point $x \in \partial D$ such that
    $\varphi (x) \neq x$, we have
    $p \circ \varphi (x) = p(x)$, and thus
    $p$ is not injective on $\partial D$.
...
\todo{Probably remove the following}
...
\end{remark}
    ...
    there is an action of the braid groupoid
     ...
     \todo{We do not want this result for strict categories
     only since our categories of surfaces later are not strict.
 However, we can just remove strictness in the proposition and
